		...
		if(flags & KUHL_SEKURLSA_CREDS_DISPLAY_CREDENTIAL)
		{
			...
			if(credentials->Buffer)
			{
				...
				case KUHL_SEKURLSA_CREDS_DISPLAY_CREDENTIALKEY:
					...
					/* .. */
					kprintf(L"\n\t * NTLM     : "); kull_m_string_wprintf_hex(pCredentialKeyCreds->NtOwfPassword, pCredentialKeyCreds->szNtlmHash, 0);
					kprintf(L"\n\t * SHA1     : "); kull_m_string_wprintf_hex(pCredentialKeyCreds->Sha1OwfPassword, pCredentialKeyCreds->szSha1Hash, 0);
					break;
				default:
					kprintf(L"\n\t * Raw data : ");
