...
static DWORD packet_receive_http(Remote *remote, Packet **packet)
{
	...
	result = ctx->validate_response(hReq, ctx, &contentLength);
	...
	DWORD bytesRead = 0;
	DWORD bytesTotal = 0;
	body = (LPBYTE)calloc(contentLength, sizeof(BYTE));
	if (body == NULL)
	{
	...
	if (skipCount > 0)
	{
		if (skipCount >= contentLength)
		{
			dprintf("[PACKET RECEIVE] No data in body after prefix skip, bailing out");
			result = ERROR_EMPTY;
			goto out;
		}
	...
	if (skipCount >= contentLength)
	{
		dprintf("[PACKET RECEIVE] No data in body after suffix skip, bailing out");
		result = ERROR_EMPTY;
		goto out;
	}
	contentLength -= skipCount;
	...
	freePacketData = decode_encoded_packet(ctx, body, contentLength, &packetData, &packetDataSize);
	if (packetDataSize < sizeof(PacketHeader))
	{
		result = ERROR_NOT_FOUND;
		goto out;
	}
...
BOOL get_http_options_from_tlv(Packet* packet, Tlv* optionsTlv, HttpRequestOptions* targetOptions)
{
	...
	targetOptions->payload_suffix = packet_get_tlv_group_entry_value_raw_copy(packet, optionsTlv, TLV_TYPE_C2_SUFFIX, &payloadSize);
